...
from constants import CHUTE_ID, CHUTE_NAME, DEPLOYMENT_ID, GPU_COUNT, SERVER_ID, SERVER_NAME

def test_purge_deployments(mock_hotkey_content, mock_client_session, mock_purge_deployments_response, tmp_path, monkeypatch, capsys):
...
def test_purge_deployment_by_id(mock_hotkey_content, mock_client_session, mock_purge_deployment_response, tmp_path, monkeypatch, capsys):
    """Test purge_deployments function."""
    # Create a temporary hotkey file
    hotkey_file = tmp_path / "hotkey.json"
    hotkey_file.write_text(mock_hotkey_content)
...
def test_purge_deployment_by_node(mock_hotkey_content, mock_client_session, mock_purge_deployment_response, tmp_path, monkeypatch, capsys):
    """Test purge_deployments function."""
    # Create a temporary hotkey file
    hotkey_file = tmp_path / "hotkey.json"
    hotkey_file.write_text(mock_hotkey_content)
...
def test_purge_deployment_by_node_name(mock_hotkey_content, mock_client_session, mock_purge_deployment_response, tmp_path, monkeypatch, capsys):
    """Test purge_deployments function."""
    # Create a temporary hotkey file
    hotkey_file = tmp_path / "hotkey.json"
    hotkey_file.write_text(mock_hotkey_content)
...
def test_purge_deployment_cli_integration(monkeypatch):
    ...
    # Run the CLI command
    result = runner.invoke(app, ["purge-deployment", "-d", DEPLOYMENT_ID, "--hotkey", "/path/to/hotkey.json"])
