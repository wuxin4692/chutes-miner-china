...
from constants import CHUTE_ID, CHUTE_NAME, DEPLOYMENT_ID, GPU_COUNT
...
def test_purge_deployment(mock_hotkey_content, mock_client_session, mock_purge_deployment_response, tmp_path, monkeypatch, capsys):
    """Test purge_deployments function."""
    # Create a temporary hotkey file
    hotkey_file = tmp_path / "hotkey.json"
    hotkey_file.write_text(mock_hotkey_content)
...
def test_purge_deployment_cli_integration(monkeypatch):
    ...
    # Run the CLI command
    result = runner.invoke(app, ["purge-deployment", DEPLOYMENT_ID, "--hotkey", "/path/to/hotkey.json"])
